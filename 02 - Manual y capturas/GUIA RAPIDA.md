# Guia rapida - Control mensual DLV

## Abrir

1. Vuelve a la carpeta principal.
2. Haz doble clic en `Control mensual DLV.html`.

## Crear acceso directo

1. Vuelve a la carpeta principal.
2. Ejecuta `Crear acceso directo.bat`.
3. Se creara `Control mensual DLV.lnk` en el escritorio.

## Respaldar datos

1. Dentro de la app, pulsa `Exportar datos`.
2. Guarda el archivo en una carpeta segura.
3. Para recuperar datos en otro computador, usa `Importar datos`.

## Carpetas

- `01 - Abrir la app`: acceso alternativo para abrir la app.
- `02 - Manual y capturas`: instrucciones e imagenes.
- `03 - Planilla Excel`: planilla complementaria.
- `04 - Sistema interno`: archivos de funcionamiento. No tocar.
